exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         if (command == 'tiktok') return client.sendButton(m.chat, json.data.video, `Jika Anda ingin mendapatkan *suara asli* tekan tombol di bawah.\n🍟 *Mengambil* : ${((new Date - old) * 1)} ms`, ``, m, [{
         ...
         if (command == 'tikwm') return client.sendButton(m.chat, json.data.videoWM, `Jika Anda ingin mendapatkan *suara asli* tekan tombol di bawah.\n🍟 *Mengambil* : ${((new Date - old) * 1)} ms`, ``, m, [{
            buttonId: `${isPrefix}tikmp3 ${args[0]}`,
            buttonText: {
               displayText: 'Audio'
            },
